...
func _cmd_help(args: Array[String]) -> void:
	...
	_add_output("  type: 1 (普通强化), 2 (终极天赋), 3 (轮回), 4 (觉醒)", Color.GRAY)
...
func _cmd_choose(args: Array[String]) -> void:
	if args.size() < 2:
		...
		_add_output("type: 1 (普通强化), 2 (终极天赋), 3 (轮回), 4 (觉醒)", Color.YELLOW)
		return
	
	var choose_type = args[1].to_int()
	if choose_type < 1 or choose_type > 4:
		_add_output("选卡类型必须是 1、2、3 或 4", Color.RED)
		return
